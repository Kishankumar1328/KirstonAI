...
export default function App() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://localhost:8000/api/v1/initials');
      if (res.ok) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await fetch(`http://localhost:8000/api/v1/initials/${id}`, { method: 'DELETE' });
      loadData();
    } catch (_) {}
  };
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <header className="flex items-center justify-between border-b border-[#1C2227] pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">Initial Management System</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <form onSubmit={handleCreate} className="p-5 rounded-2xl bg-[#0E1215] border border-[#20262B] space-y-4">
            <h2 className="text-sm font-bold text-white flex items-center gap-2">
              <Plus className="w-4 h-4 text-[#76B900]" /> New Initial
            </h2>
            <div>
          ...
          <div className="md:col-span-2 p-5 rounded-2xl bg-[#0E1215] border border-[#20262B] space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-white">Initial Records ({filtered.length})</h2>
