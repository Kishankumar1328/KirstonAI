...
export default function App() {
  ...
  const loadItems = async () => {
    setIsLoading(true);
    try {
      const res = await fetch('http://localhost:8000/api/v1/classics');
      if (res.ok) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await fetch(`http://localhost:8000/api/v1/classics/${id}`, { method: 'DELETE' });
      loadItems();
    } catch (e) {
      console.error(e);
    }
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <header className="flex items-center justify-between border-b border-[#242A2E] pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold">Classic Management Platform</h1>
              <p className="text-xs text-gray-500">Autonomous full-stack application for classic arcade game.</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <form onSubmit={handleCreate} className="p-5 rounded-2xl bg-[#15191C] border border-[#242A2E] space-y-4">
            <h2 className="text-sm font-bold text-white flex items-center gap-2">
              <Plus className="w-4 h-4 text-[#76B900]" /> Create Classic
            </h2>
            <div>
          ...
          <div className="md:col-span-2 p-5 rounded-2xl bg-[#15191C] border border-[#242A2E] space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-white">Classic Records ({filtered.length})</h2>
